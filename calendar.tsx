...
const CalendarComp = () => {
    ...
    const { currentMonthFirstDay = null, generateMonthDateList = [] } = generateMonthData(dayjsDate)
    const { currenWeekFirstDay = null, generateWeekDateList = [] } = generateWeekData(dayjsDate)
    ...
    //更新月日历视图
    const updateMounthView = (index: number, currentFirstDay, dateList: Array<dayjsType>): void => {
        setMoveIndex(index)
    ...
    //更新周日历视图
    const updateWeekView = (index: number, currentFirstDay: dayjs.Dayjs, dateList: Array<dayjsType>): void => {
        setMoveIndex(index)
    ...
    const handelSetWeekIndex = (selectDate: string | dayjs.Dayjs, generateWeekDateList: Array<{ [key: string]: any }>): void => {
        ...
        let weekIndex = 0
        currentWeekDateList.forEach((v, index) => {
    ...
    return <div className='calendar_comp'>
        ...
        <img onClick={() => {
            handelIsMountView()
        }} className='calendar_pull' src={`/images/${isMountView ? 'calendar_push' : 'calendarpull'}.svg`} alt="" />
